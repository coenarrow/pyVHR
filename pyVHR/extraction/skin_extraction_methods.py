...
class SkinExtractionRectangle:
    def __init__(self, device='CPU'):
        self.device = device
        self.rect = None
        self.mean_rgb = None

    def extract_skin(self, image, rgb_threshold):
        if self.mean_rgb is None:
            display_image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            self.rect = cv2.selectROI('Draw Rectangle on a Frame', display_image)
            cv2.destroyWindow('Draw Rectangle on a Frame')

            x, y, w, h = self.rect
            cropped_region = image[y:y+h, x:x+w]

            # Calculate mean R, G and B values within the rectangle
            self.mean_rgb = np.mean(cropped_region, axis=(0, 1))

        # Define bounds for R, G, and B using the mean values and threshold for each channel
        lower_bound = [self.mean_rgb[i] - rgb_threshold for i in range(3)]
        upper_bound = [self.mean_rgb[i] + rgb_threshold for i in range(3)]

        # Apply the threshold per channel over the entire image
        mask = np.all(np.logical_and(lower_bound <= image, image <= upper_bound), axis=-1)
        skin_image = np.where(mask[..., None], image, 0)
