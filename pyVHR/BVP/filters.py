import numpy as np
import scipy.sparse
from numba import prange, jit, int32, float32, float64
...
def apply_custom_filter(windowed_sig, filter_func, fps=None, params={}):
    """
    Apply a filter method to a windowed RGB signal or BVP signal. 
    ...
    Args:
        windowed_sig: list of length num_window of RGB signal as float32 ndarray with shape [num_estimators, rgb_channels, num_frames],
                      or BVP signal as float32 ndarray with shape [num_estimators, num_frames].
        filter_func: filter method that accept a 'windowed_sig' (pyVHR implements some filters in pyVHR.BVP.filters).
        params (dict): useful parameters passed to the filter method.
    
    Returns:
        A tuple containing a filtered signal with the same shape as the input signal and a list of IDs for frames that pass the threshold.
    """
    ...
    filtered_windowed_sig = []
    good_ids_list = []  # To store the IDs
    ...
    for idx in range(len(windowed_sig)):
        ...
        # Check if filt_temp is a tuple and separate data and IDs
        if isinstance(filt_temp, tuple):
            filt_data, good_ids = filt_temp
            good_ids_list.append(good_ids)
        else:
            filt_data = filt_temp
            
        if transform:
            filt_data = np.squeeze(filt_data, axis=1)
        
        filtered_windowed_sig.append(filt_data)

    return filtered_windowed_sig, good_ids_list  # Return the filtered signal and the list of good IDs
...
@jit([Tuple((int32[:], int32[:]))(float32[:,:,:], int32, int32), 
      Tuple((int32[:], int32[:]))(float64[:,:,:], int32, int32), 
      Tuple((int32[:], int32[:]))(int32[:,:,:], int32, int32)], 
     nopython=True, nogil=True, parallel=True, fastmath=True)
def kernel_rgb_filter_th_with_ids(sig, RGB_LOW_TH, RGB_HIGH_TH):
    """
    This method performs the Color Threshold filter for RGB signal only.
    It also returns the IDs of the frames that pass the threshold.
    """
    goodidx = np.ones((sig.shape[0],), dtype=np.int32)
    for idx in prange(sig.shape[0]):
        b_in = 1
        for f in prange(sig.shape[2]):
            if ((sig[idx][0][f] < RGB_LOW_TH and sig[idx][1][f] < RGB_LOW_TH and sig[idx][2][f] < RGB_LOW_TH) or
                ...
                b_in = 0
        goodidx[idx] = b_in
    good_ids = np.argwhere(goodidx).flatten().astype(np.int32)
    return goodidx, good_ids

def rgb_filter_th_with_ids(sig, **kargs):
    """
    Color Threshold filter for RGB signal only. Also returns the IDs of the frames
    that pass the threshold.

    Parameters are: {'RGB_LOW_TH':int, 'RGB_HIGH_TH':int}.
    Where 'RGB_LOW_TH' and 'RGB_HIGH_TH' are RGB values ([0,255]) representing the filter thresholds.
    """
    goodidx, good_ids = kernel_rgb_filter_th_with_ids(sig, np.int32(kargs['RGB_LOW_TH']), np.int32(kargs['RGB_HIGH_TH']))
    if np.sum(goodidx) == 0:
        raise Exception(f"Too tight thresholds {(kargs['RGB_LOW_TH'], kargs['RGB_HIGH_TH'])} for pixel colors, blank face mask!")
    return np.copy(sig[np.argwhere(goodidx).flatten()]), good_ids